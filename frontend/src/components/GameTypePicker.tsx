...
interface Props {
   ...
   */
  fillHeight?: boolean;
  /** Category ids omitted from chips and the type list (e.g. experimental on keep-going). */
  excludeCategories?: readonly GameTypeCategoryId[];
}
...
export function GameTypePicker({
  ...
  const categories = useMemo(
    () => GAME_TYPE_CATEGORIES.filter((cat) => !excluded.has(cat.id)),
    [excluded]
  );
  const availableTypes = useMemo(
    () => ALL_GAME_TYPES.filter((id) => !excluded.has(GAME_TYPE_BY_ID[id].category)),
    [excluded]
  );

  const filtered = useMemo(() => {
    const q = query.trim().toLowerCase();
    const activeCategory = category !== "all" && excluded.has(category) ? "all" : category;
    return availableTypes.filter((id) => {
  ...
  }, [query, category, availableTypes, excluded]);
  ...
  const chipCategory = category !== "all" && excluded.has(category) ? "all" : category;
  ...
  return (
    <div className={rootClass}>
      ...
      <div className="-mx-1 flex shrink-0 gap-1.5 overflow-x-auto px-1 pb-1 bingo-scrollbar">
        <button
          type="button"
          onClick={() => setCategory("all")}
          className={cn(
            "shrink-0 rounded-full border px-2.5 py-1 text-xs font-medium transition-colors",
            chipCategory === "all" ? "text-white" : "border-border text-muted-foreground"
          )}
          style={chipCategory === "all" ? { backgroundColor: accent, borderColor: accent } : undefined}
        >
          All
        </button>
        {categories.map((cat) => (
          <button
            ...
            className={cn(
              "shrink-0 rounded-full border px-2.5 py-1 text-xs font-medium transition-colors",
              chipCategory === cat.id ? "text-white" : "border-border text-muted-foreground"
            )}
            style={chipCategory === cat.id ? { backgroundColor: accent, borderColor: accent } : undefined}
          >
            {cat.label}
          </button>
        ))}
      ...
      <div
        className={cn("overflow-y-auto overscroll-contain rounded-lg border bingo-scrollbar", listHeightClass)}
        role="listbox"
      ...
      <p className="shrink-0 pt-0.5 text-[11px] text-muted-foreground tabular-nums">
        {filtered.length} of {availableTypes.length} types
      </p>
    </div>
  );
}
